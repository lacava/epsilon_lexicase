...
\medskip
\noindent {\bf Definition 2:} The {\it Pareto front} of $\mathcal{N}$ is the subset of $\mathcal{N}$ that is non-dominated with respect to $\mathcal{T}$; i.e., $n \in \mathcal{N}$ is on the Pareto front if $n \nprec m \; \forall \; m \in \mathcal{N}$. 
\bigskip

\medskip
\noindent {\bf Definition 3:} $n \in \mathcal{N}$ is a {\it Pareto front boundary} if $n$ is on the Pareto front of $\mathcal{N}$ and $\exists j \in \{1,\dots,T\}$ for which $e_j(n_1) \leq e_j(m) \; \forall \; m \in \mathcal{N}$. \bigskip
\bigskip
