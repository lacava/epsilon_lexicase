...
\medskip
\noindent {\bf Definition:} $n_1$ $\epsilon$-dominates $n_2$, i.e., ${n_1} \prec_{\epsilon} {n_2}$, if $e_j(n_1) + \epsilon_j \leq e_j(n_2)  \;
...
Tower & 25 & 2195 &  940 \\
UBall5D & 5 & 1024 & 5000 \\ 
Yacht	& 6	&	216	&	93	\\ \midrule
\end{tabularx}
\rowcolors{3}{white}{LightCyan}
\begin{tabularx}{\columnwidth}{L{0.25\columnwidth} L{0.3\columnwidth} r} 
\multicolumn{3}{c}{Dynamical Systems\hspace{1em}280 Training Cases, 120 Test Cases}\\ \midrule
Problem & Equations & Initial Conditions	\\ \midrule
...
Glider & $\dot{v} = - 0.05 \cdot  v^2 - sin (\theta)$	, $ \dot{\phi} = 0.5 \cdot \sin (\phi - \theta) - \sin (\phi)$	& \\
Lotka-Volterra interspecies dynamics & $\dot{x} = 3  \cdot x - 2  \cdot x \cdot y - x^2$, $ \dot{\theta} = v - \cos (\theta)/v$	&  \\
Predator Prey & $\dot{x} = x  \cdot \left( 4 - x - \frac{y}{1+x} \right)$, $ \dot{y} = 2 \cdot y - x \cdot y - y^2$ 	&  \\
Shear Flow & $\dot{\theta} = \cot (\phi) \cdot cos(\theta)$, $ \dot{\phi} = \left(\cos ^2 (\phi) + 0.1 \cdot  \sin^2 (\phi)\right) \cdot sin(\theta)$	&  \\
van der Pol oscillator & $\dot{x} = 10 \cdot  \left(y - \frac{1}{3} \cdot (x^3-x) \right)$	, $ \dot{y} = -\frac{1}{10} \cdot x$		&  \\ \midrule
\end{tabularx}
...
\caption{The textbook ODE problems (left). The models generated by \texttt{Base} and ELGP variant \texttt{EHC5} are shown on the right.} \label{tbl:real_ode_models}
\rowcolors{3}{white}{LightCyan}
\begin{tabularx}{\textwidth}{L{0.12\textwidth} L{0.25\textwidth} L{0.25\textwidth} L{0.25\textwidth} } \toprule 
 System & Target & Base Most Frequent Solution & EHC5 Most Frequent Solution \\ \midrule
Bacterial Respiration & $\dot{x} = 20 - x - \frac{x \cdot y}{1+0.5 \cdot x^2}$	&	$\dot{x} = -(7.006\cdot (x + 0.4722\cdot y - 31.11))/y$	&	$\dot{x} = 20.12 - 1.009\cdot x - \frac{1.979\cdot y}{x} $	\\
& $ \dot{y} = 10 - \frac{x \cdot y}{1+0.5 \cdot x^2}$	&	$\dot{y} = 9.994 - 0.5669\cdot y\cdot sin(3.526/x)$	&	$\dot{y} = 10.0 - \frac{2.0 \cdot x \cdot y}{2.001 + x^2}$	\\ \midrule
Bar Magnets & $\dot{\theta} = 0.5 \cdot \sin (\theta - \phi) - \sin (\theta)$	&	$\dot{\theta} = sin(sin(cos(cos(\theta) - \phi) + sin(\theta) - 3.65))$	&	$\dot{\theta} = 0.5\cdot sin(\theta - \phi) - sin(\theta)$	\\
& $ \dot{\phi} = 0.5 \cdot \sin (\phi - \theta) - \sin (\phi)$	&	$\dot{\phi} = - 0.5\cdot sin(\theta - \phi) - sin(\phi)$	&	$\dot{\phi} = - 0.5\cdot sin(\theta - \phi) - sin(\phi)$	\\ \midrule
Glider & $\dot{v} = - 0.05 \cdot  v^2 - sin (\theta)$	&	$\dot{v} = - 0.05\cdot v^2 - sin(\theta)$	&	$\dot{v} = - 0.05\cdot v^2 - sin(\theta)$	\\
& $ \dot{\theta} = v - \cos (\theta)/v$	&	$\dot{\theta} = v - (cos(\theta))/v$	&	$\dot{\theta} = v - (cos(\theta))/v$	\\ \midrule
Lotka-Volterra interspecies dynamics & $\dot{x} = 3  \cdot x - 2  \cdot x \cdot y - x^2$	&	$\dot{x} = 3.0  \cdot x - 2.0  \cdot x \cdot y - x^2$	&	$\dot{x} = 3.0  \cdot x - 2.0  \cdot x \cdot y - x^2$	\\
& $ \dot{y} = 2 \cdot y - x \cdot y - y^2$	&	$\dot{y} = 2.0 \cdot y - x \cdot y - y^2$	&	$\dot{y} = 2.0 \cdot y - x \cdot y - y^2$	\\ \midrule
Predator Prey & $\dot{x} = x  \cdot \left( 4 - x - \frac{y}{1+x} \right)$	&	$\dot{y} = -0.5674\cdot x\cdot (y + x\cdot cos (\frac{0.7896\cdot y}{x + cos(0.1632\cdot x)}) - 6.119)$	& $\dot{y} = 0.3516\cdot x \cdot \left(8.122 - \frac{x}{cos(x/y)} - y \right)$	\\
& $\dot{y} = y \cdot \left( \frac{x}{1+x} - 0.075 \cdot y \right)$	&	$\dot{y} = (y + 0.01059)\cdot \left(\frac{x}{x + 1.004}-0.07488\cdot y \right)$	&	$\dot{y} = y\cdot \left(\frac{x}{1.0 + x}-0.075\cdot y \right)$	\\ \midrule
Shear Flow & $\dot{\theta} = \cot (\phi) \cdot cos(\theta)$	&	$\dot{\theta} = \cot (\phi) \cdot cos(\theta)$	&	$\dot{\theta} = \cot (\phi) \cdot cos(\theta)$	\\
& $ \dot{\phi} = \left(\cos ^2 (\phi) + 0.1 \cdot  \sin^2 (\phi)\right) \cdot sin(\theta)$	&	$\dot{\phi} = 0.45\cdot sin(\theta)\cdot (cos(2.0\cdot \phi) + 1.222)$	&	$\dot{\phi} = 2.076\cdot sin \left(cos(sin(\phi)) - 0.4918 \right)\cdot sin(\theta)$	\\ \midrule
van der Pol oscillator & $\dot{x} = 10 \cdot  \left(y - \frac{1}{3} \cdot (x^3-x) \right)$	&	$\dot{x} = (y + 10.41)\cdot (2.232\cdot sin(x) - 1.879\cdot x + sin(y))$	& $\dot{x} = 10.0 \cdot \left(y - 0.333\cdot (x^3-x) \right)$	\\
& $ \dot{y} = -\frac{1}{10} \cdot x$	&	$\dot{y} = -0.1\cdot x$	&	$\dot{y} = -0.1\cdot x$	\\ \midrule
\bottomrule
\end{tabularx}
\end{table}
